  ...
  export default function Campo() {
    ...
    function desfazerJogada() {
      if (historicoTabuleiros.length <= 1) {
        return;
      }
    ...
    return (
      <>
        ...
        <div>
          ...
          <button onClick={desfazerJogada} disabled={historicoTabuleiros.length <=1}>Desfazer Jogada</button>
        </div>
        <div>
        <h3>Histórico de jogadas:</h3>
        <ul>{historicoTexto.map((jogada, index) => (<li key={index}>{jogada}</li>))}</ul>
        </div>
      </>
    );
  }
